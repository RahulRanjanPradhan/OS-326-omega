...
static void args_passing(void **esp);
...
process_execute (const char *cmdline)
{
  ...
  /* Split string arguments and store it into array named args.
   The format is like this:
   args[] = {"arg1\0", "arg2\0",...,"argn\0"} */
  argc = 0;
       ...
       token = strtok_r (NULL, " ", &save_ptr))
  {
    ...
    if (argc >= 50)
    {
      // Let's allow a maximum of 50 arguments.
...
process_exit (void)
{
  ...
  //Delete all the child processes information
...
/* Arguments Passing. */
static void
args_passing(void **esp)
{
  int i;
  int length = 0;
  char *args_on_stack[argc];                   // Array of addresses of args.

  // Push the arguments into stack.
  // And Store the address of arguments into array: args_on_stack.
  for (i = 0; i < argc; i++)
  {
  ...
  // Align the word.
  int word_align = 4 - (length) % 4;
  for (i = word_align; i > 0; i--)
  {
    *esp -= 1;
    memcpy(*esp, &args_on_stack[argc], 1);
  }

  // Push args[i] into stack, including null pointer and args.
  for (i = argc; i >= 0; i--)
  {
    *esp -= sizeof(char *);
    memcpy(*esp, &args_on_stack[i], sizeof(char *));
  }
